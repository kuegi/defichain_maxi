...
export class StoreConfig implements IStore {
    ...
    private GetConfig(): ConfigFile {
        ...
        if (!fs.existsSync(configfile)) { // write empty config if not exists
            fs.writeFileSync(configfile, JSON.stringify(new ConfigFile(), null, 2));
            throw new Error(`new empty config created: ${configfile}. Enter your values before the next start. Set seedfile to an encrypted folder.`)
        }
