...
import { Ocean, sendToS3 } from './utils/helpers'
...
async function analyseDay(o: Ocean, tstampStartOfDay: number): Promise<AnalysisData> {
  ...
  const startHeight = await o.getBlockForTstamp(tstampStartOfDay, refBlock)
  const endHeight =
    tstampEndOfDay > refBlock.tstamp ? refBlock.height : await o.getBlockForTstamp(tstampEndOfDay, refBlock)
