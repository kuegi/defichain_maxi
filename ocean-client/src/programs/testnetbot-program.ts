...
export class TestnetBotProgram extends CommonProgram {
    ...
    async checkAndDoArbitrage(telegram: Telegram): Promise<boolean> {
        ...
        const feeIn = this.combineFees([dusdPool.tokenB.fee?.inPct, dusdPool.tokenA.fee?.outPct,
        dusdusdtPool.tokenA.fee?.inPct, dusdusdtPool.tokenB.fee?.outPct,
        usdtPool.tokenB.fee?.inPct, usdtPool.tokenA.fee?.outPct,
        usdtPool.commission, dusdusdtPool.commission, dusdPool.commission])
        const feeOut = this.combineFees([dusdPool.tokenA.fee?.inPct, dusdPool.tokenB.fee?.outPct,
        dusdusdtPool.tokenB.fee?.inPct, dusdusdtPool.tokenA.fee?.outPct,
        usdtPool.tokenA.fee?.inPct, usdtPool.tokenB.fee?.outPct,
        usdtPool.commission, dusdusdtPool.commission, dusdPool.commission])
